...
**Palette robustness — NEVER paint the raw `--brand-*` token onto a character-bearing surface.** The brand color arrives arbitrary per salon (vivid, muted, dark, light); route it through token-layer clamps that guarantee the variant's *character*, not just legibility. `color-mix` can floor *lightness* but cannot raise *chroma* while preserving hue — for that, studio/neon use relative-color syntax `oklch(from var(--brand-x) clamp(MIN,l,MAX) max(c,FLOOR) h)` inside `@supports (color: oklch(from white l c h))`, with a plain `color-mix` fallback declared first in `@theme`. Neon splits primary into roles (atelier's two-token lesson): `--color-primary-vivid` (bright — large text/marquee, AA-large 3:1), `--color-primary-fill` (darker — small white-text pills/CTAs, AA 4.5), `--color-primary-deep` (full-bleed grounds), plus `--color-on-primary` tracking the *fill* (not `--brand-surface`, which could be dark). Studio floors its single accent + directional `--accent-on-light` (accent text on paper) / `--accent-on-ink` (accent text on the dark booking section). **Any palette change must be re-measured:** `pnpm -F @revivo/verify exec tsx bin/measure-contrast.ts` renders the real token expressions in headless Chromium and computes WCAG across muted/saturated/pale/dark/pink palettes — the eye misses small-text-on-bright-fill failures the measurement catches.

**Per-variant Services patterns (pure CSS, no JS required):** atelier = JS tabs (its Layout has the script); neon = a radio-group **pill-tab filter** (adjacent-sibling `:checked ~ .svc-panel` shows the active panel — static, unbreakable; `:has()` only for the cosmetic active-pill highlight); studio = CSS **column-flow** on desktop (balances uneven category counts, no empty cells) that becomes a **checkbox accordion** on mobile (controls `display` directly — do NOT use `<details>` with a desktop force-open, its content hides via `content-visibility` which a `display` override can't reopen).

**Screenshots:** `bin/shoot-variants.ts --base <slug> [--server-url http://localhost:PORT]` writes detail-preserving viewport-bands per variant×viewport to `packages/verify/out/critique/` (full-page reads downscale to mush). Pass `--server-url` to reuse a running `pnpm -F @revivo/mockups dev` instead of spinning a competing server.
